	...
	public void deplacerCarteInfluenceMainVersColonne(int indexMain, int indexColonne) throws Exception{
    	...
    	if(plateau.getColonne(indexColonne).nombreCartesInfluences() >= 2) {
    		...
    		if(carte instanceof CarteSpeciale) {
        		activerCapaciteCarteSpeciale((CarteSpeciale) carte);
        	}
    	}
//    	CarteInfluence carte = plateau.getColonne(indexColonne).getCartesInfluences()[plateau.getColonne(indexColonne).nombreCartesInfluences() - 2];
	...
	/**
     * Active la capacité spéciale d'une CarteSpeciale à effet immédiat.
     * 
     * @param carte Carte spéciale à activer.
     * 
     * @since 1.0
     */
    public void activerCapaciteCarteSpeciale(CarteSpeciale carte) throws Exception{
    	if(!(carte instanceof CarteARetardement) && carte != null && !carte.estDesactivee()) {
    		if(carte instanceof Traitre) {
        		...
        		carte.activer(this);
        		if(!(carte instanceof Explorateur))
     ...
     * @since 1.0
     */
    public Data(Joueur master, int nbjoueur, String difficulte) {
        ...
        for(int i = 1; i<nbjoueur; i++) {
        	Joueur bot = new Bot(difficulte, colorInt.get(i), "bot"+i);
        	this.addJoueur(bot);
        }
    ...
    public int resultatFinManche(int numeroColonne){
    	...
    	for(CarteInfluence carte : this.getPlateau().getColonne(numeroColonne).getCartesInfluences()) {
    		if(carte instanceof Prince) {
    			if(((Prince)carte).isGagne()==true) {
    				numeroVainqueur=this.getIndexJoueurParCouleur(carte.getCouleur());
    				System.out.println("Joueur : " + numeroVainqueur + "gagne avec le couple Prince et Ecuyer");
    	...
    	for(int k=0; k<resultats.length; k++) {
    		...
    		if (resultats[k]>resultats[numeroVainqueur]) {
        		...
        		for(CarteInfluence carte : this.getPlateau().getColonne(numeroColonne).getCartesInfluences()) {
        			if(carte instanceof Mendiant) {
        				if(resultats[k]<resultats[numeroVainqueur] && resultats[k]!=0) {
        					numeroVainqueur = k;
        				}
        			}
        		}
	...
	public void finDeManche() {
		for(int i = 0; i<plateau.getColonnes().length; i++) {
			...
			plateau.getColonne(i).getCarteObjectif().setManche(this.getCurrentManche());
